...
The console requires an OpenAI API key (**user key** or **project key**) with Realtime API access. You'll be prompted to enter it at startup. It is saved via `localStorage` and can be updated through the UI. Since the api is still in beta, you may have varying levels of success. My hope is that, in the long term, this becomes more stable. Also, **watch your costs on the OpenAI billing page.**
